...
这类成熟提示词的价值不在于"助手是谁"，而在于"它如何决定做什么"。这套决策机制在任何 agent、任何宿主、任何任务间都可复用。Agent Harness 就是把这套机制蒸馏出来。
...
这个套件经过三轮刻意迭代生长而成，每一轮都更深地触及源提示词的设计哲学。
...
Agent Harness 让 agent 行为变得**模块化、可验证、可移植**。
...
Agent Harness 是**提示词编译器，不是沙箱**。宿主仍然控制工具、权限、网络和记忆。宿主指令始终优先于编译后的 harness 文本。原始 Claude Fable 5 源提示词仅作为设计语料保留——绝不复制进编译后的运行时提示词。
